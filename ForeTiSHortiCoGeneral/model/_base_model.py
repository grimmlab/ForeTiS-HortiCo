...
class BaseModel(abc.ABC):
    ...
    Every model must be based on :obj:`~ForeTiSHortiCo-Hortico.model._base_model.BaseModel` directly or
    BaseModel's child classes, e.g. :obj:`~ForeTiSHortiCo-Hortico.model._sklearn_model.SklearnModel` or
    :obj:`~ForeTiSHortiCo-Hortico.model._torch_model.TorchModel`
    ...
    ** Attributes **
        ...
        * Instance attributes *
        ...
        - all_hyperparams (*dict*): dictionary with all hyperparameters with related info that can be tuned (structure see :obj:`~ForeTiSHortiCo-Hortico.model._base_model.BaseModel.define_hyperparams_to_tune`)
    ...
    def __init__(self, optuna_trial: optuna.trial.Trial, datasets: list, featureset: str,
                 pca_transform: bool, target_column: str):
        ...
        if featureset == 'optimize':
            ...
            for dataset in datasets.datasets:
                if dataset.name == dataset_name:
                    ...
                    break
            for retailer_dataset in datasets.retailer_datasets:
                if retailer_dataset.name == dataset_name:
                    self.retailer_dataset = retailer_dataset
                    break
        else:
            for dataset in datasets.datasets:
                if dataset.name == featureset:
                    ...
                    break
            for retailer_dataset in datasets.retailer_datasets:
                if retailer_dataset.name == featureset:
                    self.retailer_dataset = retailer_dataset
                    break
    ...
    def suggest_hyperparam_to_optuna(self, hyperparam_name: str):
        ...
        :param hyperparam_name: name of the hyperparameter to be tuned (see :obj:`~ForeTiSHortiCo-Hortico.model._base_model.BaseModel.define_hyperparams_to_tune`)
    ...
    def save_model(self, path: str, filename: str):
        ...
        (can be loaded with :obj:`~ForeTiSHortiCo-Hortico.model._model_functions.load_model`)
