...
def get_list_of_implemented_models() -> list:
    ...
    if os.path.exists('../model'):
        ...
        model_src_files = os.listdir('model')
    elif os.path.exists('ForeTiSHortiCoGeneral/model'):
        model_src_files = os.listdir('ForeTiSHortiCoGeneral/model')
    else:
        model_src_files = [model_file + '.py' for model_file in ForeTiSHortiCoGeneral.model.__all__]
...
def get_mapping_name_to_class() -> dict:
    ...
    if os.path.exists('../model'):
        ...
        files = os.listdir('ForeTiSHortiCoGeneral/model')
    else:
        files = [model_file + '.py' for model_file in ForeTiSHortiCoGeneral.model.__all__]
    modules_mapped = {}
    for file in files:
        if file not in ['__init__.py', '__pycache__']:
            ...
            file_name = file[:-3]
            module_name = 'ForeTiSHortiCoGeneral.model.' + file_name
...
def get_indexes(df: pd.DataFrame, n_splits: str, datasplit: str):
    ...
    if datasplit == 'timeseries-cv':
        ...
        for idx in range(len(year_list) - train_len):
            ...
            if type(train_df_indexes[0]) == int:
                train_df_indexes = list(range(train_df_indexes[0], train_df_indexes[-1].stop))
            else:
                train_df_indexes = list(range(train_df_indexes[0].start, train_df_indexes[-1].stop))
            ...
            test_df_indexes = list(range(test_df_indexes[0].start, test_df_indexes[-1].stop))
