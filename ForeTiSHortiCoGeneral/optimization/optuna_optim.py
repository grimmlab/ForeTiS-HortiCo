...
class OptunaOptim:
    ...
    ** Attributes **
        ...
        - dataset (*obj:`~ForeTiSHortiCo-Hortico.preprocess.base_dataset.Dataset*): dataset to use for optimization run
    ...
    def objective(self, trial: optuna.trial.Trial):
        ...
        try:
            model: _base_model.BaseModel = helper_functions.get_mapping_name_to_class()[self.current_model_name](
                target_column=self.target_column, datasets=self.datasets,
                featureset=self.featureset, optuna_trial=trial, pca_transform=self.pca_transform,
                **additional_attributes_dict)
        ...
        self.retailer_dataset = model.retailer_dataset
    ...
    def generate_results_on_test(self) -> dict:
        ...
        if self.test_set_size_percentage == 'yearly':
            test = self.retailer_dataset.loc[str(self.datasets.test_year) + '-01-01': str(self.datasets.test_year) + '-12-31']
        ...
        for count, period in enumerate(self.user_input_params["periodical_refit_cycles"]):
            ...
            if hasattr(final_model, 'sequential'):
                test = self.retailer_dataset.tail(len(test) + final_model.seq_length)
            ...
            if 'lstm' in self.current_model_name:
                test = self.retailer_dataset.tail(test_len)
