...
from .raw_data_functions import custom_resampler, drop_columns, get_one_hot_encoded_df
...
class Dataset:
    ...
    **Attributes**
    ...
    def __init__(self, data_dir: str, data: str, config_type: str, target_column: str,
                 windowsize_current_statistics: int, windowsize_lagged_statistics: int, cyclic_encoding: bool = False,
                 correlation_number: int = None, correlation_method: str = None,
                 config: configparser.ConfigParser = None, test_year: int = None, retailer: str = None):
        ...
        # if os.path.exists(os.path.join(data_dir, 'general.h5')):
        #     datasets = list()
        #     with pd.HDFStore(os.path.join(data_dir, 'general.h5'), 'r') as hdf:
        #         keys = hdf.keys()
        #         for key in keys:
        #             dataset = pd.read_hdf(os.path.join(data_dir, 'general.h5'), key=key)
        #             dataset.name = key[1:]
        ...
        for datafile in data:
            ...
            # load raw data
            dataset_raw = self.load_raw_data(data_dir=data_dir, data=datafile)
            ...
            dataset_raw = dataset_raw.loc[: str(test_year) + '-12-31']
            ...
            df = self.featureadding_and_resampling(df=dataset_raw, test_year=test_year)
            ...
            r1 = re.compile(".*seaslag1.*")
            if list(filter(r1.match, list(df))):
                drop_columns(df=df, columns=['stat_' + self.target_column + '_seaslag1',
                                             'stat_' + self.target_column + '_seaslag1_rolling_mean3',
                                             ])
            r2 = re.compile(".*seaslag2.*")
            if list(filter(r2.match, list(df))):
                drop_columns(df=df, columns=['stat_' + self.target_column + '_seaslag2',
                                             'stat_' + self.target_column + '_seaslag2_rolling_mean3'
                                             ])
            ...
            train_yrs = year_list[:train_len]

            df["mean_" + self.target_column] = df.loc[df.index.year.isin(train_yrs), :][self.target_column].mean()

            datafiles.append(df)
            print('---Data preprocessed---')

        df = pd.concat(datafiles, sort=False).sort_index()
        ...
        # filename_h5 = os.path.join(self.data_dir, 'general.h5')
        ...
        retailer_df = datafiles[data.index(retailer)]
        if 'cal_Winterferien_Counter' not in retailer_df:
            retailer_df['cal_Winterferien_Counter'] = 99
        if 'cal_Pfinstferien_Counter' not in retailer_df:
            retailer_df['cal_Pfingstferien_Counter'] = 99
        if 'cal_Allerheiligen_Counter' not in retailer_df:
            retailer_df['cal_Allerheiligen_Counter'] = 99
        if hasattr(self, 'correlations'):
            for column in retailer_df.columns:
                if re.search('stat_correlations.*', column):
                    self.correlations += [column]
        dataset_weather = pd.concat([retailer_df.filter(regex='whi'), retailer_df[self.target_column]], axis=1)
        dataset_weather.name = 'dataset_weather'
        dataset_cal = pd.concat([retailer_df[self.target_column], retailer_df.filter(regex='cal'),
                                 retailer_df.filter(regex='^holiday_school')], axis=1)
        dataset_cal.name = 'dataset_cal'
        dataset_sales_corr = pd.concat([retailer_df[self.target_column], retailer_df.filter(regex='stat')], axis=1) \
            .drop(list(retailer_df.filter(regex='stat_whi')), axis=1)
        dataset_sales = dataset_sales_corr.copy()
        if hasattr(self, 'correlations'):
            for column in self.correlations:
                dataset_sales.drop(list(dataset_sales.filter(regex=column)), axis=1, inplace=True)
        ...
        dataset_cal_sales.name = 'dataset_cal_sales'
        if hasattr(self, 'correlations'):
            dataset_full_corr = retailer_df.copy()
            dataset_full_corr.name = 'dataset_full_corr'
        dataset_full = retailer_df.copy()
        if hasattr(self, 'correlations'):
            for column in self.correlations:
                dataset_full.drop(list(dataset_full.filter(regex=column)), axis=1, inplace=True)
        dataset_full.name = 'dataset_full'
        ...
        if hasattr(self, 'correlations'):
            retailer_datasets = [dataset_weather, dataset_cal, dataset_sales, dataset_sales_corr,
                                 dataset_weather_sales,
                                 dataset_weather_sales_corr, dataset_weather_cal, dataset_cal_sales,
                                 dataset_cal_sales_corr,
                                 dataset_full, dataset_full_corr]
        else:
            retailer_datasets = [dataset_weather, dataset_cal, dataset_sales, dataset_weather_sales,
                                 dataset_weather_cal,
                                 dataset_cal_sales, dataset_full]

        self.datasets = datasets
        self.retailer_datasets = retailer_datasets
    ...
    def featureadding_and_resampling(self, df: pd.DataFrame, test_year: int) -> list:
        """
