...
def get_list_of_implemented_models() -> list:
    ...
    if os.path.exists('../model'):
        ...
        model_src_files = os.listdir('model')
    elif os.path.exists('ForeTiSHortico/model'):
        model_src_files = os.listdir('ForeTiSHortico/model')
    else:
        model_src_files = [model_file + '.py' for model_file in ForeTiSHortico.model.__all__]
...
def get_mapping_name_to_class() -> dict:
    ...
    if os.path.exists('../model'):
        ...
        files = os.listdir('ForeTiSHortico/model')
    else:
        files = [model_file + '.py' for model_file in ForeTiSHortico.model.__all__]
    modules_mapped = {}
    for file in files:
        if file not in ['__init__.py', '__pycache__']:
            ...
            file_name = file[:-3]
            module_name = 'ForeTiSHortico.model.' + file_name
